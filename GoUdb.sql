...
-- Schema GoUdb
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `GoUdb` DEFAULT CHARACTER SET utf8 ;
USE `GoUdb` ;
...
CREATE UNIQUE INDEX `cedula_UNIQUE` ON `GoUdb`.`usuarios` (`id_usuario` ASC);
...
CREATE TABLE IF NOT EXISTS `GoUdb`.`referidos` (
  `id_usuario` INT(20) NOT NULL,
  `cod_referido` VARCHAR(255) NOT NULL,
  `caducidad` DATETIME NOT NULL,
  `cant_referidos` INT NOT NULL,
  PRIMARY KEY (`id_usuario`, `cod_referido`),
  CONSTRAINT `FK_id_usuario_referidos`
    FOREIGN KEY (`id_usuario`)
    REFERENCES `GoUdb`.`usuarios` (`id_usuario`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

CREATE UNIQUE INDEX `codigo_UNIQUE` ON `GoUdb`.`referidos` (`cod_referido` ASC);
...
CREATE TABLE IF NOT EXISTS `GoUdb`.`conductores` (
  ...
  `foto_pase` TINYTEXT NOT NULL,
  `foto_cedula` TINYTEXT NOT NULL,
  `cod_referido` VARCHAR(255) NOT NULL,
  `cuenta_activada` TINYINT(1) NOT NULL DEFAULT 1,
  PRIMARY KEY (`codigo_asoc`, `id_conductor`),
  CONSTRAINT `FK_codigo_referido`
    FOREIGN KEY (`cod_referido`)
    REFERENCES `GoUdb`.`referidos` (`cod_referido`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE UNIQUE INDEX `id_conductor_UNIQUE` ON `GoUdb`.`conductores` (`id_conductor` ASC);
...
CREATE TABLE IF NOT EXISTS `GoUdb`.`vehiculos` (
  ...
  `foto_soat` TINYTEXT NOT NULL,
  `foto_tarj_prop` TINYTEXT NOT NULL,
  `foto_tecno` TINYTEXT NOT NULL,
  PRIMARY KEY (`placa`),
...
CREATE TABLE IF NOT EXISTS `GoUdb`.`rutas` (
  ...
  CONSTRAINT `FK_conductor`
    ...
    REFERENCES `GoUdb`.`conductores` (`id_conductor`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;
...
CREATE INDEX `FK_conductor_idx` ON `GoUdb`.`rutas` (`id_conductor` ASC);
...
CREATE INDEX `FK_usuario_idx` ON `GoUdb`.`servicios` (`id_usuario` ASC);

CREATE INDEX `FK_conductor_servicio_idx` ON `GoUdb`.`servicios` (`id_conductor` ASC);
...
CREATE TABLE IF NOT EXISTS `GoUdb`.`auditoria_usuarios` (
  `id_usuario` INT(20) NOT NULL,
  `tipo_sancion` INT NOT NULL DEFAULT 0,
...
CREATE UNIQUE INDEX `id_usuario_UNIQUE` ON `GoUdb`.`tokens_conductores` (`id_conductor` ASC);
...
-- -----------------------------------------------------
-- Table `GoUdb`.`tipos_sanciones`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `GoUdb`.`tipos_sanciones` (
...
CREATE DEFINER = CURRENT_USER TRIGGER `GoUdb`.`rutas_AFTER_INSERT` AFTER INSERT ON `rutas` FOR EACH ROW
    BEGIN
		INSERT INTO `log_rutas` (tipo_registro, fecha, id_ruta, id_conductor, origen, destino, opcion_parada, tarifa_destino, hora_sal, hora_lleg, cupos, historial_viaje) VALUES ("Creación", now(), NEW.id_ruta, NEW.id_conductor, NEW.origen, NEW.destino, NEW.opcion_parada, NEW.tarifa_destino, NEW.hora_sal, NEW.hora_lleg, NEW.cupos, NEW.historial_viaje);  
        END$$

USE `GoUdb`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GoUdb`.`rutas_AFTER_UPDATE` AFTER UPDATE ON `rutas` FOR EACH ROW
     BEGIN
		INSERT INTO `log_rutas` (tipo_registro, fecha, id_ruta, id_conductor, origen, destino, opcion_parada, tarifa_destino, hora_sal, hora_lleg, cupos, historial_viaje) VALUES ("Modificación", now(), NEW.id_ruta, NEW.id_conductor, NEW.origen, NEW.destino, NEW.opcion_parada, NEW.tarifa_destino, NEW.hora_sal, NEW.hora_lleg, NEW.cupos, NEW.historial_viaje);  
        END$$

USE `GoUdb`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GoUdb`.`rutas_BEFORE_DELETE` BEFORE DELETE ON `rutas` FOR EACH ROW
     BEGIN
		INSERT INTO `log_rutas` (tipo_registro, fecha, id_ruta, id_conductor, origen, destino, opcion_parada, tarifa_destino, hora_sal, hora_lleg, cupos, historial_viaje) VALUES ("Eliminación", now(), OLD.id_ruta, OLD.id_conductor, OLD.origen, OLD.destino, OLD.opcion_parada, OLD.tarifa_destino, OLD.hora_sal, OLD.hora_lleg, OLD.cupos, OLD.historial_viaje);  
        END$$

USE `GoUdb`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GoUdb`.`servicios_AFTER_INSERT` AFTER INSERT ON `servicios` FOR EACH ROW
    BEGIN
		INSERT INTO `log_servicios` (tipo_registro, fecha, id_servicio, id_ruta, id_usuario, id_conductor, parada, tarifa_automatica, cancelar_servicio) VALUES ("Creación", now(), NEW.id_servicio, NEW.id_ruta, NEW.id_usuario, NEW.id_conductor, NEW.parada, NEW.tarifa_automatica, NEW.cancelar_servicio);  
        END$$

USE `GoUdb`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GoUdb`.`servicios_AFTER_UPDATE` AFTER UPDATE ON `servicios` FOR EACH ROW
    BEGIN
		INSERT INTO `log_servicios` (tipo_registro, fecha, id_servicio, id_ruta, id_usuario, id_conductor, parada, tarifa_automatica, cancelar_servicio) VALUES ("Modificación", now(), NEW.id_servicio, NEW.id_ruta, NEW.id_usuario, NEW.id_conductor, NEW.parada, NEW.tarifa_automatica, NEW.cancelar_servicio);  
        END$$

USE `GoUdb`$$
CREATE DEFINER = CURRENT_USER TRIGGER `GoUdb`.`servicios_BEFORE_DELETE` BEFORE DELETE ON `servicios` FOR EACH ROW
    BEGIN
		INSERT INTO `log_servicios` (tipo_registro, fecha, id_servicio, id_ruta, id_usuario, id_conductor, parada, tarifa_automatica, cancelar_servicio) VALUES ("Eliminación", now(), OLD.id_servicio, OLD.id_ruta, OLD.id_usuario, OLD.id_conductor, OLD.parada, OLD.tarifa_automatica, OLD.cancelar_servicio);  
        END$$
