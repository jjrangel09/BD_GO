...
CREATE DEFINER = CURRENT_USER TRIGGER `GoUdb`.`auditoria_conductores_BEFORE_DELETE` BEFORE DELETE ON `auditoria_conductores` FOR EACH ROW
    BEGIN
    INSERT INTO `log_calificaciones` (tipo_registro, fecha, id_usuario, tipo_sancion, acumulado, cantidad_calificaciones, estado_usuario) VALUES ("Eliminación", now(), OLD.id_conductor, OLD.sanciones_tipo, OLD.acumulado, OLD.cantidad_calificaciones, OLD.estado_conductor);
    END $$
