...
CREATE TABLE IF NOT EXISTS `GoUdb`.`auditoria_usuarios` (
  ...
  CONSTRAINT `FK_id_auditoria_usuario`
    ...
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `FK_tipo_sancion_usuarios`
    FOREIGN KEY (`tipo_sancion`)
    REFERENCES `GoUdb`.`tipos_sanciones` (`id_tipo_sancion`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE INDEX `FK_tipo_sancion_usuarios_idx` ON `GoUdb`.`auditoria_usuarios` (`tipo_sancion` ASC);
...
CREATE TABLE IF NOT EXISTS `GoUdb`.`auditoria_conductores` (
  ...
  CONSTRAINT `FK_id_conductor_auditoria`
    ...
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `FK_tipo_sancion_conductor`
    FOREIGN KEY (`tipo_sancion`)
    REFERENCES `GoUdb`.`tipos_sanciones` (`id_tipo_sancion`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE INDEX `FK_tipo_sancion_conductor_idx` ON `GoUdb`.`auditoria_conductores` (`tipo_sancion` ASC);
